...
from src_20m.models import GRPOMoE
...
def train_epoch(model, train_loader, optimizer, device, args):
    ...
    for batch in tqdm(train_loader, desc="Training"):
        ...
        # 3. 그룹 상대 보상 계산 (GRPO 특징)
        group_rewards = model.compute_group_relative_rewards(rewards)

        # 4. GRPO 업데이트 (배치를 grpo_epochs번 반복 학습)
        model.train()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train GRPO-MoE model (ml-20m)")
    ...
    # GRPO Training
    parser.add_argument("--batch_size", type=int, default=1024, help="Batch size for GRPO updates")
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="checkpoints_20m/grpo_moe", help="Checkpoint directory")
